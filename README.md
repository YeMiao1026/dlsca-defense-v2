...
**D05（通用擾動模板，Gu et al. 2020風格）已正式跑完，結果是目前對Route A論述最乾淨的一組證據**：`src/generator/universal_perturber.py` 新增單一可訓練向量的架構（輸出跟輸入軌跡內容無關，對每條軌跡廣播同一個學出來的波形）。正式跑（PSR=0.0098，只要D01成本的39%）：`GE@9000=162.46`，比隨機基準還高34.96，且隨N持續遞增（不是打平）——單看GE，會誤以為這個防禦比D01（GE=111.84）更有效、成本還更低。但SNR/TVLA診斷顯示它對兩者的改變**精確是0.0%**——這不是實驗巧合，是可以直接證明的數學必然：對每條軌跡加同一個常數擾動，SNR公式（類間變異數/類內變異數）跟TVLA的Welch t-test對常數平移都完全不變。這代表GE看起來的「贏」跟真正的安全性可以完全脫鉤，而且這裡不需要靠實驗歸納，靠數學就能斷言這個防禦對安全性毫無貢獻——比D01/高斯的比較更適合當方法論教學案例。

**D06（自適應攻擊者A1）已跑完，D01的防禦在重訓練下幾乎完全失效**：攻擊端新增`01_train_attacker.py --profiling-traces`，用D01防禦後的全部50000條Profiling_traces重新訓練一個攻擊者。跟frozen E01（GE@9000=111.84）比較，同一批D01防禦後的E集：自適應攻擊者**GE@9000=0.00、N_TGE=596、100次獨立攻擊全部成功**——而完全無防禦時的N_TGE是475，代表D01的防禦一旦攻擊者知道它存在並重新訓練，幾乎不構成任何實質障礙。跟D05的SNR/TVLA診斷合起來，是目前對Route A核心論點（「失效是資訊理論/威脅模型層級的，不是工程細節」）支撐力道最強的兩組獨立證據。21天路線圖的四個核心項目（TVLA、D03重分析、D05通用模板、D06自適應攻擊者）全部完成，下一步轉向整理報告草稿。詳見 `CLAUDE.md` 附錄D/E。
