...
**目前狀態：D01＋D02（loss權重掃描）已跑完**。D01的誠實結果是負面的：跟同PSR成本的高斯噪訊基準線比，移植的GAN產生器效率明顯更差（`GE@9000=111.84` vs 高斯噪訊在同PSR下 `GE@9000=0.00`）。D02原本猜測是loss的smooth/l2懲罰把擾動壓得太平滑太一致、被攻擊者的逐點統計量吸收——四點掃描（拿掉/降低這兩項懲罰）結果完全相反：正則化越弱，成本越高、GE也越差，D01自己（滿正則化）反而是五個設定裡唯一贏過隨機基準的。目前懷疑真正的效率差距來自「確定性擾動 vs 高斯噪訊的獨立隨機性」這個結構性差異，還待驗證。詳見 `CLAUDE.md` 附錄A（文件末尾）跟 [`docs/runs.md`](./docs/runs.md)。
