...
Also doubles as the "informed adaptive attacker" realignment step: passing the
full (50000-trace) jamming-defended Profiling_traces instead of the E-set
produces a realigned profiling set an attacker could retrain on -- testing
whether resync-ing the *training* data (not just the evaluation data) lets an
adaptive attacker recover from a training collapse caused by unaligned data.

Usage (run from repo root):
    # E-set (static A0 re-evaluation):
    ...
    # full profiling set (informed adaptive retraining):
    python scripts/07_check_resync_bypass.py \
        --attack-repo ~/dlsca-attack-v2 \
        --attacker-run ~/dlsca-attack-v2/runs/E01_baseline_clean_20260816_1302 \
        --jammed-traces defenses_d08_combo/D04_jam20_profiling_.../combined_profiling_traces.npy \
        --max-shift 20 --out-dir defenses_d08_resync
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Realign jamming-defended traces (E-set or full profiling set) and measure "
                     "how much attack strength returns"
    )
    ...
    p.add_argument("--jammed-traces", required=True,
                    help=".npy of jamming-defended traces to realign -- either the E-set "
                         "(10000 traces, for a static A0 re-evaluation) or the full "
                         "Profiling_traces (50000 traces, to retrain an informed adaptive "
                         "attacker on already-realigned data)")
...
def main() -> None:
    ...
    is_profiling = len(jammed) != len(a_idx) and len(jammed) > 20000
    kind = "profiling set (50000 traces)" if is_profiling else "E-set"
    print(f"=== realigning {len(jammed)} jammed {kind} traces (max_shift={args.max_shift}) ===")
    ...
    out_name = "resynced_profiling_traces.npy" if is_profiling else "resynced_traces.npy"
    out_path = out_dir / out_name
    np.save(out_path, aligned.astype(np.float32))
    print(f"=== saved {out_path} ===")
    print()
    if is_profiling:
        print("=== next: retrain an informed adaptive attacker on this realigned profiling set ===")
        print("  python3 scripts/01_train_attacker.py --config configs/exp/E01_adaptive_A1_retrain.yaml "
              f"--profiling-traces {out_path} --runs-dir runs_adaptive_sweep")
    else:
