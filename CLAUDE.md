...
- **如果要繼續**，方向已經比A.7原本模糊的「對抗樣本 vs 資訊破壞」更具體、更可操作：loss需要加一項明確要求「同時壓低所有已知洩漏點的SNR/類間可分性」（不是只讓單一凍結模型的單次輸出出錯），例如把`src/metrics/leakage.py::snr`（或其可微分版本）直接放進訓練loss，強制產生器在point 517跟156兩處都要有感——這是loss設計層級的改動，不是本次D01-D03這種權重/隨機性掃描能碰到的，工作量比D02/D03大不少。
- D04（原D03，PSR成本匹配三方比較圖）仍不建議現在做——目前最好的配方（D01）在成本匹配下還是明顯輸給裸高斯噪訊，且現在已經知道確切原因（洩漏點覆蓋不完整），做比較圖前應該先驗證「加入SNR-aware loss項」是否真的能補上這個缺口。
