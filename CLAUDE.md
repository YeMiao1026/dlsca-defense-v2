...
- **建議的下一步**：用攻擊端`00_inspect_data.py`已有的SNR健檢邏輯（或直接呼叫`src/metrics/leakage.py::snr`），對D01防禦後的E集（`defended_traces.npy`）跟同PSR的高斯噪訊防禦後的E集，各自算一次逐點SNR，跟乾淨E集比較峰值下降幅度。這能直接證實或推翻A.7的「對抗樣本不等於資訊破壞」假說，且不需要重新訓練任何東西，成本很低。
- 如果A.7成立，代表要讓GAN防禦真正比高斯噪訊有效率，loss設計需要從「讓攻擊者單次query出錯」換成「直接壓低目標時間點的SNR/類間可分性」這類跟資訊本身掛鉤的目標，而不是單純的confuse-against-uniform交叉熵——這是loss函數設計層級的改動，不是本次D01-D03這種權重/隨機性掃描能碰到的。
- D04（原D03，PSR成本匹配三方比較圖）仍不建議現在做——目前最好的配方（D01）在成本匹配下還是明顯輸給裸高斯噪訊，七次系統性嘗試都沒能縮小差距；應該先驗證A.7、或視驗證結果重新設計loss，至少找到一個能在某個PSR區間打平或贏過高斯基準線的配方，再進入三方比較。
