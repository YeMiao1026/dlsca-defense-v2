...
- **D02**：對 loss 權重（尤其 `lambda_smooth`、`lambda_l2`）跟 `epsilon` 做掃描，檢驗 A.3 的「擾動太平滑/太一致」假設——如果拿掉或降低 smoothness 懲罰後 PSR-vs-GE 效率明顯改善，就支持這個假設；如果沒有明顯差異，代表問題更根本（例如 loss 本身的 confusion 目標設計，或需要引入 per-trace 隨機性）。
- 目前不建議直接跳去做 D03（PSR成本匹配三方比較圖）——D01 這個配方在成本匹配下明顯輸給裸高斯噪訊，此時做比較圖只會呈現「GAN比噪訊差」，不是專案原本想驗證的方向；應該先讓 D02 至少找到一個在某個 PSR 區間打平或贏過高斯基準線的配方，再進入三方比較。
